...
public class CustomDAO {
	...
	public CustomDAO() {
		...
		try {
			...
			conn = DriverManager.getConnection(
					"jdbc:mariadb://localhost:3306/dump1", "dump", "zxcv1234");
		} catch (Exception e) {
	...
	public int insert(CustomVO vo) {
		...
		String sql = "INSERT INTO custom\r\n"
				...
				+ "VALUES ('"+vo.getBusi_num()+"', '"+vo.getCustom()+"', '"+vo.getShort_name()+"', '"+vo.getCeo()+"', '"+vo.getCharge_person()+"', '"+vo.getBusi_condition()+"', '"+vo.getItem()+"', '"+vo.getPost_num()+"', '"+vo.getAddr1()+"', '"+vo.getAddr2()+"', '"+vo.getTel()+"', '"+vo.getFax()+"', '"+vo.getHomepage()+"', '"+vo.getCo_yn()+"', '"+vo.getForeign_yn()+"', '"+vo.getTax_yn()+"', '"+vo.getCountry_eng()+"', '"+vo.getCountry_kor()+"', '"+vo.getSpecial_relation()+"', '"+vo.getTrade_stop()+"', '"+vo.getContract_period_s()+"', '"+vo.getContract_period_e()+"', '"+vo.getRegi_info_man()+"', NOW(), '"+vo.getModi_info_man()+"', NOW())";
		try {
	...
	public CustomVO selectOne(String busi_num) {
		...
		try {
			stmt = conn.createStatement();
			rs = stmt.executeQuery("SELECT * FROM custom c LEFT JOIN account a ON c.BUSI_NUM=a.BUSI_NUM WHERE c.BUSI_NUM='"+busi_num+"'");
			if (rs.next()) {
				vo = new CustomVO();
